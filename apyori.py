...
OrderedStatistic = namedtuple( # pylint: disable=C0103
    'OrderedStatistic', ('items_base', 'items_add', 'confidence', 'lift', 'support_base'))
...
def gen_ordered_statistics(transaction_manager, record):
    ...
    for base_length in range(len(items)):
        for combination_set in combinations(sorted_items, base_length):
            ...
            support_base = transaction_manager.calc_support(items_base)            
            yield OrderedStatistic(
                frozenset(items_base), frozenset(items_add), confidence, lift, support_base)
